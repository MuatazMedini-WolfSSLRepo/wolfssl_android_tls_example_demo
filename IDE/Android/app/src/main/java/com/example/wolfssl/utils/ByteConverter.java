package com.example.wolfssl.utils;
...
public class ByteConverter {
    ...
    public static float convertByteArrayToFloat(byte[] input) {
        return ByteBuffer.wrap(input).order(ByteOrder.BIG_ENDIAN).getFloat();
    }
    ...
    public static int getBitValue(int bitValue) {
        if (bitValue > 0) {
            int base = 2;
            int exponent = bitValue;
            bitValue = 1;
            while (exponent != 0) {
                bitValue *= base;
                --exponent;
